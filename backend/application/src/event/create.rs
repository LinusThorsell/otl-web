...
pub fn create_event(event_data: NewEvent) -> Result<(), Box<dyn std::error::Error>> {
    match insert_into(events).values((title.eq(event_data.title), date.eq(event_data.date), url.eq(event_data.url), tour_id.eq(event_data.tour_id))).get_result::<Event>(&mut establish_connection()) {
        Ok(_) => { Ok(()) },
        Err(err) => match err {
